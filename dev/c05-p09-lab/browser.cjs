// Command-driven Playwright worker for the joint C05 + P09 acceptance. It is the P09 worker with one
// addition: the student arrives through the real C05 landing page instead of having a session placed
// for them, and the editor then opens in that same browser context. Credentials arrive on stdin;
// nothing is written outside the evidence directory.
...
const commands = {
  ...
  async open(command) {
    // `keep` continues in the context the student already landed in: the joint chain is exactly that
    // one browser, carrying the session the C05 consume page created.
    const page = command.keep ? state.page : await fresh(command.viewport);
    if (!command.keep) await session(page, command.token, command.user_id);
  ...
  // The student arrives: the real landing page spends the one-time handoff and the session that comes
  // back is the one the rest of this chain uses. Nothing is placed in storage by the harness.
  async consume(command) {
    ...
    await page.waitForTimeout(command.wait || 3000);
    ...
    if (command.screenshot) await shot(command.screenshot);
    return { ok: true, path: url.pathname + url.search, url_has_handoff: url.searchParams.has('handoff'),
