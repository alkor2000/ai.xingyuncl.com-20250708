...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '准备成果（开发验证）' }).waitFor();
    assert.equal(await page.getByRole('button', { name: '准备成果（开发验证）' }).count(), 1);
    await page.getByRole('button', { name: '准备成果（开发验证）' }).click();
    ...
    await page.screenshot({ path: path.join(output, 'desktop-scope.png'), fullPage: true });
    await page.getByRole('button', { name: '确认范围并固定快照' }).click();
    await page.getByText('快照已固定，尚未交接', { exact: true }).waitFor();
    const downloadPromise = page.waitForEvent('download');
    await page.getByRole('button', { name: '下载草案清单与内容 .json' }).click();
    ...
    await page.getByRole('button', { name: '模拟发送 / 恢复同一操作' }).click();
    await page.getByText('接收结果未确认，请恢复同一操作', { exact: true }).waitFor();
    await page.screenshot({ path: path.join(output, 'response-lost.png'), fullPage: true });
    // Reload browser, resume the server-side snapshot and reconcile the already accepted operation.
    await page.reload();
    await page.getByRole('button', { name: '准备成果（开发验证）' }).click();
    await page.getByRole('button', { name: '恢复上次快照' }).click();
    await page.getByText('接收结果未确认，请恢复同一操作', { exact: true }).waitFor();
    await page.getByRole('button', { name: '模拟发送 / 恢复同一操作' }).click();
    await page.getByText('模拟接收方已接收；未写入 TE-DNA', { exact: true }).waitFor();
    ...
    await page.screenshot({ path: path.join(output, 'mobile-recovered.png'), fullPage: true });
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth));
    assert.deepEqual(errors, []);
    assert.deepEqual(external, []);
    await fs.writeFile(path.join(output, 'browser-result.json'), JSON.stringify({ passed: true, checks: [
      'assistant-only-entry', 'exact-selected-text', 'explicit-owned-attachment', 'download-packet-isolation',
