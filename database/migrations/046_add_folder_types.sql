...
-- 4. 创建全局文件夹（如果不存在）
INSERT IGNORE INTO user_folders (user_id, folder_type, name, path, created_at)
VALUES (1, 'global', '全局共享', '/全局共享', NOW());

-- 5. 为每个现有组创建组织文件夹
INSERT INTO user_folders (user_id, folder_type, group_id, name, path, created_at)
SELECT 
    (SELECT id FROM users WHERE role = 'admin' AND group_id = g.id LIMIT 1) as user_id,
