...
CREATE TABLE IF NOT EXISTS calendar_events (
  ...
  parent_event_id BIGINT COMMENT '父事项ID（重复事项）',
...
CREATE TABLE IF NOT EXISTS calendar_categories (
  ...
  user_id BIGINT COMMENT '用户ID（NULL表示系统预设）',
...
CREATE TABLE IF NOT EXISTS calendar_ai_analyses (
  ...
  model_id BIGINT COMMENT 'AI模型ID',
...
CREATE TABLE IF NOT EXISTS calendar_user_settings (
  ...
  default_model_id BIGINT COMMENT '默认AI模型ID',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
DROP PROCEDURE IF EXISTS sp_cleanup_old_calendar_events;
DELIMITER //
CREATE PROCEDURE sp_cleanup_old_calendar_events()
BEGIN
...
-- 10. 创建触发器：自动创建用户设置
DROP TRIGGER IF EXISTS trg_create_calendar_settings;
DELIMITER //
CREATE TRIGGER trg_create_calendar_settings
