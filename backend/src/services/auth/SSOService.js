 ...
 * 2. MD5签名验证 - uuid + timestamp + shared_secret
 * 3. IP白名单（可选）- 限制来源IP
 * 
 * 签名算法说明：
 * 当前使用 MD5，因为需要兼容已有的外部对接方
 * 如果新增对接方，建议协商使用 SHA-256
 */
...
class SSOService {
   ...
   * 验证SSO请求
   * 
   * @param {Object} params - { uuid, timestamp, signature }
   * @param {string} clientIp - 客户端IP地址
   * @returns {Object} SSO配置对象
   ...
   * @param {Object} ssoConfig - SSO配置对象
   * @returns {Object} 用户实例
