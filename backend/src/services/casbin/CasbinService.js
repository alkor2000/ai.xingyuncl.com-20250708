...
class CasbinService {
  ...
  async initialize() {
    try {
      ...
      // 创建deny策略表（如果不存在）
      await this.createDenyPolicyTable();
   ...
   * 创建deny策略表
   */
  async createDenyPolicyTable() {
    const sql = `
      CREATE TABLE IF NOT EXISTS casbin_deny_policies (
      ...
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Casbin拒绝策略表'
    `;
    
    try {
      await dbConnection.query(sql);
      logger.info('Deny策略表已准备就绪');
    } catch (error) {
      logger.error('创建deny策略表失败:', error);
    }
  }
  ...
  async createMySQLAdapter() {
    ...
    const adapter = {
      loadPolicy: async (model) => {
        try {
          const sql = 'SELECT * FROM casbin_rule ORDER BY id';
          ...
          logger.info(`加载了 ${rows.length} 条策略`);
        } catch (error) {
  ...
  loadPolicyLine(row, model) {
    ...
    } else if (row.ptype === 'g' || row.ptype === 'g2') {
      // 角色继承策略 - 修正：使用正确的方法名
      model.addGroupingPolicy('g', row.ptype, lineArray);
    }
  }
  ...
  async checkPermission(subject, object, action) {
    ...
    try {
      // 1. 首先检查是否有deny策略
      const isDenied = await this.checkDenyPolicy(subject, object, action);
      if (isDenied) {
        logger.debug('Casbin权限检查（被拒绝）', {
          subject, object, action, result: false
        });
        return false;
      }
   ...
   * 检查deny策略
   */
  async checkDenyPolicy(subject, object, action) {
    try {
      // 检查直接deny
      const sql = `
        SELECT COUNT(*) as count 
        FROM casbin_deny_policies 
        WHERE subject = ? AND object = ? AND action = ?
      `;
      const { rows } = await dbConnection.query(sql, [subject, object, action]);
      
      if (rows[0].count > 0) {
        return true;
      }
      
      // 检查继承的deny（需要获取用户的所有角色）
      if (this.enforcer) {
        const roles = await this.enforcer.getRolesForUser(subject);
        for (const role of roles) {
          const { rows: roleRows } = await dbConnection.query(sql, [role, object, action]);
  ...
  async addPolicy(subject, object, action, effect = 'allow') {
    ...
    try {
      if (effect === 'deny') {
        // 添加deny策略到单独的表
        const sql = `
          INSERT INTO casbin_deny_policies (subject, object, action)
          VALUES (?, ?, ?)
          ON DUPLICATE KEY UPDATE created_at = NOW()
        `;
        await dbConnection.query(sql, [subject, object, action]);
        return true;
      } else {
        // 添加allow策略到casbin
   ...
   * 删除策略
   */
  async removePolicy(subject, object, action, effect = 'allow') {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      if (effect === 'deny') {
        // 删除deny策略
        const sql = `
          DELETE FROM casbin_deny_policies 
          WHERE subject = ? AND object = ? AND action = ?
        `;
        await dbConnection.query(sql, [subject, object, action]);
        return true;
