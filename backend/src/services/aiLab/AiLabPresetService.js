 ...
 *
 * 职责：
 * - 列出 / 读取包：解析并规范化 manifest（图像包 files、表格包 columns + rows），带 mtime 缓存
 ...
 *     · 表格：行样本 file_path=NULL，payload 按数据集 columns 规范化
 *     · origin_ref = <pack_key>:<包内相对路径>（表格行为 <pack_key>:rows/train/<i> 或 rows/shift/<set>/<i>），
 ...
 *     · 事务写入样本 + sample_count + 数据集变更；失败则删除已落盘的图片
 */
...
const { isValidPackKey, isSafeRelativePath, resolvePackFile, selectPerClass, computeCounts } = presetPacks;
...
class AiLabPresetService {
  ...
  static normalizeManifest(raw, dirKey) {
    ...
    const kind = raw.kind === 'table' ? 'table' : (raw.kind === 'image' ? 'image' : null);
    if (!kind) throw new ValidationError(`预置包 ${dirKey} 的 kind 只能是 image 或 table`);
    ...
    if (kind === 'image') {
      ...
      pack.columns = AiLabDataset.validateColumns(raw.columns || []);
  ...
  static summarize(pack) {
    const { files, rows, dir, ...rest } = pack; // eslint-disable-line no-unused-vars
  ...
  static async nextFileIndex(absoluteDir, packKey) {
    ...
    try {
      ...
      const pattern = new RegExp(`^preset-${packKey}-(\\d+)\\.jpg$`);
      names.forEach(name => {
  ...
  static async importPack({ userId, dataset, pack, perClass = null, shiftSets = [], includeTrain = true }) {
    ...
    if (dataset.kind !== pack.kind) {
      ...
      columns = pack.kind === 'table' ? pack.columns : [];
      datasetUpdates.columns = pack.kind === 'table' ? columns : null;
    ...
    /* 3. 表格行按最终 columns 再规范化一次（columns 合并后可能多了列，键仍需在其中） */
    if (pack.kind === 'table') {
      items.forEach(item => {
      ...
      await dbConnection.transaction(async (query) => {
        if (items.length > 0) {
          await AiLabSample.insertMany(items.map((item, index) => ({
            ...
            source: 'preset',
            origin_ref: item.ref,
            file_path: pack.kind === 'image' ? stored[index].file_path : null,
            width: pack.kind === 'image' ? stored[index].width : null,
            height: pack.kind === 'image' ? stored[index].height : null,
            file_size: pack.kind === 'image' ? stored[index].file_size : null,
            payload: pack.kind === 'table' ? item.payload : null,
