...
class CalendarService {
  ...
  static buildAnalysisPrompt(events, scanDateStart, scanDateEnd) {
    ...
    const prompt = `你是一位专业的时间管理顾问和日程规划专家。请分析用户的日历事项,提供深度洞察和优化建议。

【数据概览】
- 分析时间范围：${scanDateStart} 至 ${scanDateEnd}
- 事项总数：${stats.total}
- 分类分布：${Object.entries(stats.by_category).map(([k, v]) => `${k}(${v})`).join('、')}
- 状态分布：已完成(${stats.by_status.completed || 0})、进行中(${stats.by_status.in_progress || 0})、未开始(${stats.by_status.not_started || 0})、日常(${stats.by_status.daily || 0})
- 重要度分布：高优(${stats.by_importance.high})、中等(${stats.by_importance.medium})、低优(${stats.by_importance.low})
...
请从以下5个维度进行深度分析，用Markdown格式输出，包含表格、列表、emoji图标：

## 📊 1. 任务优先级排序
- 列出Top 10高优任务（重要度>=8）
- 用表格展示：日期、任务内容、重要度、状态、分类
- 标注紧急程度和建议处理顺序
...
- 用饼图数据展示分类分布
- 评估时间分配是否合理，是否有失衡现象
- 提供具体的时间分配建议（如工作50%、学习30%、生活20%）

## ⚠️ 3. 任务冲突与风险检测
- 标注同一天内有多个高优任务（importance>=8）的日期
- 用表格列出冲突日期、任务数量、任务列表
- 分析可能的时间冲突和资源冲突
- 提供解决方案（如任务重排、优先级调整）
...
- 适当使用emoji增强可读性`;
  ...
  static async performAnalysis(userId, options) {
    try {
      ...
      // 9. 扣除积分 - 修复：使用正确的 transaction_type
      await user.consumeCredits(
        creditsNeeded,
        model_id,
        null,
        `日历AI分析 - ${events.length}个事项 - ${model.display_name}`,
        'calendar_analysis'  // 修复：改为 calendar_analysis
      );
