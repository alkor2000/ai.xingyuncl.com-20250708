...
const INSTRUCTIONS = {
  ...
  docx: [
    ...
    '5. 如果用户要的是通知、函、请示、报告等公文：第一行用 # 写公文标题；下一段单独写主送机关并以中文冒号结尾（如"各年级组、各处室："）；正文分段，层级标题按"一、""（一）""1."的公文习惯写在段首；如有附件，用单独一段以"附件："开头列出；结尾另起两段分别写发文单位和成文日期（如"二〇二六年九月十五日"）。平台会据此把内容套进学校自己的公文模板。',
    ...COMMON_TAIL
  ],
  pdf: [
    '【输出格式要求：PDF 文档】',
