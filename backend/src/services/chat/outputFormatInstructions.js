...
const INSTRUCTIONS = {
  pptx: [
    ...
    '1. 把整份演示文稿放在一个以 ```pptx 开头、以 ``` 结尾的代码块里（语言标识必须是 pptx）；代码块之外只写一两句简短说明，不要附带 Python 生成脚本或其他代码块（平台会直接生成 .pptx 文件）。',
    ...
    '7. 页数按内容需要决定，一般 6–15 页。公式和化学方程式不要用 LaTeX（$$…$$），直接写成普通文字并用 Unicode 上下标，如：6CO₂ + 6H₂O →（光能、叶绿体）C₆H₁₂O₆ + 6O₂、E = mc²。',
  ...
  docx: [
    ...
    '1. 把整份文档放在一个以 ```docx 开头、以 ``` 结尾的代码块里（语言标识必须是 docx）；代码块之外只写一两句简短说明，不要附带生成脚本或其他代码块（平台会直接生成 .docx 文件）。',
    ...
    '4. 内容要完整、可直接使用，不要写"此处省略"之类的占位。公式不要用 LaTeX（$$…$$），直接写成普通文字并用 Unicode 上下标（如 H₂O、x²）。',
    ...COMMON_TAIL
  ],
  pdf: [
    '【输出格式要求：PDF 文档】',
