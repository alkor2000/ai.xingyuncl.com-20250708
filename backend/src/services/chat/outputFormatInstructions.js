...
const INSTRUCTIONS = {
  pptx: [
    ...
    '6. 不要使用图片，除非用户给出了图片 URL；代码块内部不要再出现 ``` 围栏，需要展示代码时用 ~~~ 围栏。',
    '7. 页数按内容需要决定，一般 6–15 页。',
    '8. 版式会按内容自动排，请善用：同一页写 2–3 个 ## 小标题各带要点＝左右分栏（适合对比、优缺点）；一个 3–6 步的有序列表、每步一句短语＝流程图；3–6 条"- **名称**：一句说明"＝卡片网格（适合要素、概念）；整页只放一段 > 引文＝大字引言。一份课件里穿插使用 2–3 种，不要每页都是要点列表。',
    ...COMMON_TAIL
  ],
  docx: [
    '【输出格式要求：Word 文档】',
    ...
    '3. 文档内需要展示代码时用 ~~~ 作为围栏，不要用 ```。不要使用图片，除非用户给出了图片 URL。',
