...
class StorageController {
   ...
   * @param {string} action - 操作类型：upload/delete/view/rename
  ...
  static async processUploadedFiles(req, res) {
    ...
    try {
      ...
      if (uploadedFiles.length > 0 && creditCost > 0) {
        const deductResult = await StorageController.deductCredits(userId, creditCost, '文件上传');
        if (!deductResult) {
          // 积分扣除失败，理论上不应该到这里（前面已检查）
          logger.error('积分扣除失败', { userId, creditCost });
        }
      }
  ...
  static async renameFolder(req, res) {
    try {
      ...
      // 检查重命名权限
      if (!StorageController.checkFolderPermission(folder, req.user, 'rename')) {
        return ResponseHelper.forbidden(res, '无权重命名此文件夹');
      }
      
      // 执行重命名（会自动递归更新子文件夹路径）
      await folder.rename(new_name.trim());
      
      logger.info('文件夹重命名成功', {
        folderId: id,
        oldName: folder.name,
        newName: new_name.trim(),
        userId: userId
      ...
      return ResponseHelper.success(res, folder, '文件夹重命名成功');
    } catch (error) {
      logger.error('重命名文件夹失败:', error);
      return ResponseHelper.error(res, error.message || '重命名文件夹失败');
    }
  }
  ...
  static async deductCredits(userId, amount, description = '文件上传') {
    ...
    try {
      // 1. 使用 FOR UPDATE 锁定用户记录（行锁）
      const lockSql = 'SELECT credits_quota, used_credits FROM users WHERE id = ? FOR UPDATE';
      const { rows: userRows } = await transaction.query(lockSql, [userId]);
      ...
      // 3. 扣除积分（在事务中执行）
      const updateSql = 'UPDATE users SET used_credits = used_credits + ? WHERE id = ?';
      await transaction.query(updateSql, [amount, userId]);
      ...
      `;
      await transaction.query(historySql, [userId, -amount, newBalance, description]);
      
      // 6. 提交事务
      await transaction.commit();
      
      logger.info('扣除用户积分成功', { 
        userId, 
        amount, 
        description,
