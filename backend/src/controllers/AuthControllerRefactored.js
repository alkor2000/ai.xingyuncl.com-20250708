 ...
 * 
 * Bug修复记录：
 * - v1.1: 修复"纯数字用户名无法登录"问题
 *   * 问题现象：用户名允许纯数字创建（3-20位，见register/checkUsername/updateProfile
 *     四处统一的校验正则 /^[a-zA-Z0-9_-]{3,20}$/），但若该数字用户名恰好凑成
 *     "11位、以1开头、第二位是3-9"的手机号格式（很多单位的学号/工号体系正好是
 *     11位数字），login方法的账号类型自动识别逻辑会优先把它当作手机号去查
 *     User.findByPhone()，而该字符串从未作为任何人的手机号存入数据库，
 *     导致永远查不到用户、登录失败；只有用户名以字母开头（不可能匹配纯数字的
 *     手机号正则）才会正确落入用户名分支，表现为"用数字能注册但登录不了，
 *     必须以字母开头"。
 *   * 修复方式：账号格式判断中，邮箱格式（含@）无歧义直接按邮箱查找；
 *     纯数字且匹配手机号格式的输入存在"用户名 vs 手机号"歧义，改为优先按
 *     用户名精确查找，查不到再回退按手机号查找，两种场景都能正确登录。
 */
...
class AuthControllerRefactored {
  ...
  static async login(req, res) {
    try {
      ...
      // 根据账号格式查找用户
      //
      // 说明（Bug修复v1.1）：用户名允许纯数字创建（3-20位，见register/checkUsername/
      // updateProfile四处统一的校验正则 /^[a-zA-Z0-9_-]{3,20}$/），这与"11位数字、
      // 1开头第二位3-9"的手机号格式正则存在天然歧义——若某用户的数字用户名恰好凑成
      // 这个格式（很多单位的学号/工号体系正好是11位数字），原实现会优先当作手机号
      // 查找 User.findByPhone()，而该字符串从未作为任何人的手机号存入数据库，
      // 导致永远查不到用户、登录失败（表现为"用数字能注册但登录不了，必须以字母
      // 开头才能登录"，因为字母开头必然不匹配纯数字的手机号正则）。
      //
      // 修复：邮箱格式（含@）无歧义，直接按邮箱查找；纯数字且匹配手机号格式的
      // 输入存在"用户名 vs 手机号"歧义，优先按用户名精确查找（用户名是更直接的
      // 账号标识），查不到再回退按手机号查找，确保两种场景都能正确登录。
      let user = null;
      ...
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(account)) {
        ...
        // 纯数字且符合手机号格式：可能是"数字用户名"也可能是"手机号"，存在歧义
        // 优先按用户名精确匹配，查不到再按手机号回退查找
