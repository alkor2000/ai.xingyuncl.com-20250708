...
class CalendarController {
  ...
  /**
   * 获取背景知识列表
   * GET /api/calendar/background-knowledge
   */
  static async getBackgroundKnowledge(req, res) {
    try {
      const userUuid = req.user.uuid;

      if (!userUuid) {
        return ResponseHelper.error(res, '用户UUID不存在', 400);
      }

      const knowledge = await CalendarBackgroundKnowledge.getUserKnowledge(userUuid);

      const formattedKnowledge = knowledge.map(k => k.toJSON());

      return ResponseHelper.success(res, formattedKnowledge, '获取背景知识列表成功');
    } catch (error) {
      logger.error('获取背景知识列表失败:', error);
      return ResponseHelper.error(res, error.message || '获取背景知识列表失败');
    }
  }

  /**
   * 创建背景知识
   * POST /api/calendar/background-knowledge
   */
  static async createBackgroundKnowledge(req, res) {
    try {
      const userUuid = req.user.uuid;
      ...
      const { title, content, enabled, sort_order } = req.body;

      const knowledge = await CalendarBackgroundKnowledge.create({
        title,
        content,
        enabled,
        sort_order
      ...
      return ResponseHelper.success(res, knowledge.toJSON(), '背景知识创建成功', 201);
    } catch (error) {
      logger.error('创建背景知识失败:', error);
      return ResponseHelper.error(res, error.message || '创建背景知识失败');
    }
  }

  /**
   * 更新背景知识
   * PUT /api/calendar/background-knowledge/:id
   */
  static async updateBackgroundKnowledge(req, res) {
    try {
      const { id } = req.params;
      const userUuid = req.user.uuid;
      ...
      const knowledge = await CalendarBackgroundKnowledge.update(
        parseInt(id),
        updateData,
        userUuid
      );

      return ResponseHelper.success(res, knowledge.toJSON(), '背景知识更新成功');
    } catch (error) {
      logger.error('更新背景知识失败:', error);
      return ResponseHelper.error(res, error.message || '更新背景知识失败');
    }
  }

  /**
   * 删除背景知识
   * DELETE /api/calendar/background-knowledge/:id
   */
  static async deleteBackgroundKnowledge(req, res) {
    try {
      const { id } = req.params;
      const userUuid = req.user.uuid;
      ...
      await CalendarBackgroundKnowledge.delete(parseInt(id), userUuid);

      return ResponseHelper.success(res, null, '背景知识删除成功');
    } catch (error) {
      logger.error('删除背景知识失败:', error);
      return ResponseHelper.error(res, error.message || '删除背景知识失败');
    }
  }

  /**
   * 批量调整排序
   * POST /api/calendar/background-knowledge/reorder
   */
  static async reorderBackgroundKnowledge(req, res) {
    try {
      const userUuid = req.user.uuid;
      ...
      const knowledge = await CalendarBackgroundKnowledge.reorder(items, userUuid);

      return ResponseHelper.success(
        res,
        knowledge.map(k => k.toJSON()),
        '排序更新成功'
      );
    } catch (error) {
      logger.error('更新背景知识排序失败:', error);
      return ResponseHelper.error(res, error.message || '更新排序失败');
    }
  }
