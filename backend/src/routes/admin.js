...
const calendarConfigRoutes = require('./admin/calendarConfigRoutes'); // 日历配置路由
const agentNodeTypeRoutes = require('./admin/agentNodeTypes'); // Agent节点类型管理路由
...
const UsageLogController = require('../controllers/admin/UsageLogController'); // 使用记录控制器
...
// 日历配置路由 - /api/admin/calendar/*
router.use('/calendar', calendarConfigRoutes);

// Agent节点类型管理路由 - /api/admin/agent-node-types/*
router.use('/agent-node-types', agentNodeTypeRoutes);

// 系统统计路由 - /api/admin/stats
router.use('/stats', statsRoutes);

// ===== 会话管理路由（独立路由，用于管理员查看对话内容）=====
/**
