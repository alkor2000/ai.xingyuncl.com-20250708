...
### 2026-09-21 21:xx：默认关闭的源侧正式运行时、HTTPS 业务链、受限账本迁移候选（CTRL-20260921-PRACTICE-I03-RUNTIME-01）

fc1+e1 已登记为开发/隔离联验冻结基线后，本包把既有部件组合成**独立的正式运行时**，全部默认关闭、未挂公开保存入口、无生产操作：

- **客户端分层**（`i03Client.js`）：`HandoffWireClient` 持有两种 wire 共用的协议逻辑（信封、票据/回执校验、W/R 票界），`I03DraftClient` 保持回环 development/test 传输逐字节不变（原 35 项客户端测试全过），新增 `I03FormalClient` 只讲 `teacher-artifact-handoff/1`、经固定生产传输 `I03HttpsTransport.post(action, envelope, idempotencyKey)` 发送。**编排分层**（`i03Source.js`）：`HandoffSource` 共享核心；`I03DraftSource` 原样保留 development/test 门；`I03FormalSource` 只接受正式 wire、formal 客户端、带 owner 锚锁与跨进程互斥的存储，本身无环境门——是否构造由运行时开关与就绪核验决定，**没有放宽草案的 production 拒绝**。
- **运行时装配**（`formalRuntime.js`，`server.js` 在数据库初始化后调用 `bootstrapFormalHandoff`）：`P03_HANDOFF_ENABLED` 未设/`false` → 关闭（不建连接池、不读凭据、不调对端、不影响普通入口）；`true` → 严格构造；其他任何值 → `invalid_handoff_configuration`（启动失败关闭，不静默开启）。开启时逐项核验：Identity 部署配置有效且 issuer/公开 origin/client/实例键与 `TRUST` 相等（北大站实例键须已由 enrollment 写入）→ `P03_HANDOFF_*` 六项逐字相等 → 受限账本账号（非应用账号、口令 ≥16）→ 账本就绪探测（库名相等、MySQL 8、`SHOW GRANTS` 恰为四表 `SELECT, INSERT, UPDATE, DELETE` + `USAGE`，任何 ALL PRIVILEGES/全局/他表授权 → `handoff_ledger_role_too_broad`，缺表/缺列/错库/断连各有固定码）→ 传输固定 origin、443、系统 CA。Basic 凭据由已验证的 Identity 配置派生，只发 Identity。就绪对象只含事实，不含凭据；关闭时释放连接池与清理任务。**公开保存路由未挂载。**
- **HTTPS 业务链**（`artifactHandoffFormalHttps.test.js`，5 项）：真实 `I03FormalSource` + `I03FormalClient` + `I03HttpsTransport` 对**实验 TLS 对端**（临时 CA 签发 `id.pkuailab.com`/`workflow.pkuailab.com` 证书）跑 prepare/commit 成功（Host/SNI 为固定主机名、Basic 只到 Identity、清单 protocol_version 随 wire、W 持久）、目标停顿→`target_unavailable` 不重试并可经 status 恢复、错 SAN/不受信 CA → `identity_unavailable` 且零请求字节、302 不跟随（`receipt_invalid`）、明文 origin 无法构造；传输的实验路由（`laboratory: {ca, lookup, ports}`）只在 development/test 接受，production 构造直接拒绝，`rejectUnauthorized`/`checkServerIdentity`/TLS≥1.2 不可改。原 21 项传输测试保留。
- **同版三端（源跳数走 TLS）**：`check.py --candidate=candidate-formal-https.json`——MySQL 受限角色源以**生产 `I03FormalClient` + `I03HttpsTransport`**（仅实验 CA/回环解析/实验端口）经 TLS 到真实 Identity 提供方（G7 提交 `49224e1`，overlay 加同一处理器的 TLS 前端）与真实 T11 e1 `cmd/t11-lab formal:true`（Python TLS 前端，t11-lab 本身回环）：**八场景全过**（`storage/private/p03-handoff-validation/triad-20260921-t11-formal-e1-https/result.json` SHA `6d23ef60…`，race、1959 项输入前后一致，候选固定）。目标→Identity 兑换仍为回环 HTTP（TE 自己的传输，其 RUNTIME 候选另证）。与提供方 fc1 提交 `14b9852` 的差别只有 Identity G7 的 `readiness.go` 与 `store.go` formal/draft 门控（实验路径 `EnableFormalCandidate` 行为不变），按 fc1+e1 规则为新提供方版本单独取证。
- **迁移与角色候选**：`backend/migrations-candidates/p03/20260921_001_p03_handoff_ledger.js`（knex 格式，`up` 逐字复用 `mysqlStore.SCHEMA`，`down` 按外键顺序删四表；**放在候选目录**——knex 只扫 `backend/migrations/`，北大站 `make deploy-docker` 会自动跑 `knex migrate:latest`，进入该目录即等于下次发布建表，故 G3/G4 授权前不得晋级）；单测守卫候选不在扫描目录且现有迁移不触账本表。`docs/integrations/p03-restricted-role-runbook.md`（运维手册：建表→建号→精确 GRANT→配置→只读核验 `backend/scripts/p03-ledger-readiness.cjs`→回退）。隔离演练 `dev/p03-ledger-migration-check.py`（一次性 mysql:8.0，以本地线上副本的 schema 前像 + knex_migrations 记录为底）：只有候选待执行→up→四表新增、其余 110 表不变；`SHOW CREATE TABLE` 与 `SCHEMA` 直建逐字节相等；knex 重跑/直接重跑 up 幂等；受限账号就绪 `grant_count=5`，ALL PRIVILEGES 账号/root/缺一表授权/错库/缺表分别 `too_broad`/`too_broad`/`role_missing`/`database_mismatch`/`table_missing`，受限账号 DROP/CREATE/ALTER/读 users/GRANT 全拒；四表备份→`down` 只删四表→恢复→就绪→knex 视为待执行→重跑无变化；仅预存 owners 表的库续跑完成；无 CREATE 权限的迁移账号失败（`ER_TABLEACCESS_DENIED_ERROR`）后 root 重跑完成。证据 `storage/private/p03-handoff-validation/ledger-migration/result.json`。
- **准入清单**：`docs/integrations/p03-admission-checklist.md`（实例键空/enrollment、星云 J2 禁止、影子谓词、决-12 列缺失不宣称覆盖、D04 与逐附件复制权只列现有事实与缺口、保留期只列待决、R 后显示"已在目标回收站"不续查）。

- **依赖增量 CTRL-20260921-PRACTICE-I03-RUNTIME-INPUT-01（TE r2 × Identity G7 真实 main）**：TE r2（`/home/hanying/tedna-sync/releases/20260921-t11-runtime-r2/source`，父 `a5dc5d1`，39 源，manifest `f34fc3ae…`/closure `a3593e08…`，迁移 `20260921_03` 改为先撤默认 ACL 再最小授予）与 Identity G7 记录（真实 main 五门 + 4/4，`runs-main/20260921T125717Z/result.json` SHA `e1613d85…`，运行提交 `9b6ca01`）到达后，`candidate-formal-https.json` 改钉 r2 + Identity `9b6ca01`，**源跳数走 TLS 的正式三端再次八场景通过**（`triad-20260921-t11-runtime-r2-https/result.json` SHA `60d4fab8…`，1971 项输入前后一致；目标为 r2 树的 `cmd/t11-lab` 同一字节，Tier B）。这补上了 Identity 记录里"实践源仍 loopback HTTP"的一层：本仓正式编排的两跳（Identity issue/revoke、目标 prepare/commit/status/cancel）经生产传输、TLS、主机名校验到达真实提供方与 r2 接收方。
- **过程纠正**：运行器此前以 `GOFLAGS=-mod=mod` 构建候选树的 `cmd/t11-lab`，Go 把 r2 树 `backend/go.mod` 的 `golang.org/x/net // indirect` 规范化为直接依赖（go.mod 不在 TE manifest 内，故逐文件核对未拦到；本仓输入指纹拦到了）。已把 r2 的 go.mod 恢复为与父版 a5dc5d1 导出一致的原字节（SHA `64339dca…`）并改为 `-mod=readonly` 重跑；`20260921-t11-refresh`/`-formal`/`-formal-e1` 三树的 go.mod 同为规范化形态（565 字节），其中 refresh 可能由本仓 17:57 的预演造成，formal/e1 的时间戳早于本仓运行。建议 TE 把 go.mod/go.sum 纳入 manifest。
