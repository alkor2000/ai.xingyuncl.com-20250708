...
| backend/migrations-candidates/p09/20260922_002_p09_write_sequence.js | 1c478506d628d7b36dd13351fc6e2f623ad1c031f343fd3883dbbc4c795f536c |
| backend/src/routes/websiteArtifacts.js | cf0d2bcfbf8fefbf923e3de684c8ff32b808dd02aa8fada3039d28115046eb88 |
| backend/src/services/websiteArtifact/assets.js | 0ffa5ab286e543fe9cf357f6b73105d53c64712ca9d87448ccfdd5a1e4c2e56b |
...
| docs/integrations/p09-website-artifact-source-candidate.md | f9e37b9d21c2c57919a3f63581f9c9aad1bf91cfe63517019e117dd9d323790f |
| docs/integrations/p09-e09-consumer-package.md | 90d8e68e5c055beb95f9ee21d291d5daa285307e0cb40a5d436d1999d6f7b65e |
...
20260922_002_p09_write_sequence.js[PD4T]: F:T·P09 账本候选迁移 002：只为既有库补 write_seq/applied_write_seq，两列都在即空操作，两列都缺一条语句补齐，半状态按保守方向处理 | R:code:backend/src/services/websiteArtifact/store.js,code:backend/migrations-candidates/p09/20260922_001_p09_website_artifacts.js,code:dev/p09-lab/migration-replay.py,code:dev/p09-lab/lock-wait.py | A:exports.up/down/columns | S:迁移只加缺的列，绝不替业务对账宣布完成——重复执行必须保住 write_seq 与 applied_write_seq 的差值(欠账)；半状态修复先打标记后丢无法验证的声明，applied>write 是"修复被中断"的指纹，up 进来先找它并接着做完；down 先把只存在于计数里的欠账写成标记再删列；从不清除已有 sync_pending_at；整份迁移跑在**一条固定连接**上（knex.raw 每条语句各取一条连接，池小时第二条正是本迁移握着的那条），会话级 lock_wait_timeout 设成 15s 并在连接还池前还原（MySQL 默认 31536000s，不设就不是"拿不到即拒绝"而是挂住——账本被占用时连 up 的第一次账本读取都停在 Waiting for table metadata lock，锁都还没请求）；等待超时与拿不到锁都归为同一具名拒绝 p09_exclusive_entry_unavailable，本次不写数据不加列；拒绝附带的观察只读服务端元数据(SHOW OPEN TABLES)，绝不再读被占用的账本表、也不再向池取第二条连接（原 observeActivity 两次采样正是在这两点上会把应该立刻返回的拒绝拖住）；仍在候选目录，晋级是另一次授权
...
测试与实验文件按既有 Managed Scope 只 observe：`backend/src/__tests__/**`（含本包新增的 `websiteArtifactAssets.test.js`）、`dev/p09-lab/**`、`frontend/src/__tests__/**`。
