...
**前提分别写清**：P09 关闭时（部署默认）账本没有写入，各分支都安全；P09 开启时，普通分支只是空操作或一条追加 DDL，DDL 期间被拒的 P09 写入不造成损失（学生保存不受影响、钩子吞掉、作品留在欠账里）。**半状态修复分支不同**——缺一列时运行中的服务根本没法记录写入——它不靠"文字保证"，而是**两次采样账本**（行数/最新 updated_at/事件序号），一旦发现有变动就**具名拒绝** `p09_ledger_busy_during_half_state_repair` 并且一个字节都不改，提示运维先关掉 P09 开关再跑。**迁移不自行停任何东西，也不把停用当授权。**
