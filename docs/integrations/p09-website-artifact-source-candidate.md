...
| **图片写入链（逐跳核过）** | **网页编辑器自己没有上传**：它是 Monaco 源码编辑器，全文件无 file input、无上传调用；`html_resources` 全仓**没有任何写入方**（只在 `HtmlProject` 的一处 JOIN 里被读）。学生页面里的图片都是**从别处粘来的 URL**，而那些地方都已经记了归属：`files`（`POST /api/chat/upload-image` → multer 落 `<storage>/uploads/chat-images/YYYY-MM/` → `File.create(user_id, file_path, status='ready')`，`chat.use` 是每个 user 的基础权限）、`user_files`（云盘 → `ossService`，本地模式字节在 `<storage>/uploads/<oss_key>`）、`image_generations`（AI 图像模块 → `user_id` + `local_path`，本地模式下 `local_path` 是**本域绝对 URL** `https://<domain>/uploads/<key>`）、`forum_attachments`（`user_id` + `file_path`）。`/uploads` 目录本身**不证明任何归属** | 固定版本只复制"某一行能证明属于本人"的文件；只按路径存在**一律不复制**（`ownership_unproven`）。**不新造表、不给编辑器新造上传口**：归属行本来就有，缺的是读取面。解析器现在问这四张表，并把**本部署自己域名**下的 `/uploads` 绝对 URL 当作同一个本地对象（站外 URL 仍然只登记不拉取） |
...
- 拒绝一律**具名**进 manifest 的 `refused_assets`：`ownership_unproven`（没有任何行**精确**命名这个对象）、`other_project_resource`、`remote_object_storage`（字节在对象存储，不下载）、`symlink_refused`（路径上**任何一段**是软链接，在走到它的那一刻就拒）、`path_rejected`、`outside_upload_root`、`path_anchoring_unavailable`（这台机器没有内核锚定，资产冻结按名关闭）、`file_missing`、`unsupported_type`、`asset_too_large`、`asset_limit_reached`。

**路径边界怎么保证的（以及残余）**：按路径校验是**校验不住**的——不管解析出什么，名字都能在下一次调用之前被换掉。实测的调度就是这样绕过去的：先沿父目录软链接把描述符打开（拿到根外的文件），在 `realpath` 之前把父目录换成真目录（于是路径比较通过），再在 `stat` 之前换回软链接（于是 inode 比较也通过），根外字节照样返回。所以现在**不再校验路径，而是走路径**：从上传根的描述符开始，**每一段都通过上一段的描述符打开**（`/proc/self/fd/<dirfd>/<名字>`，内核按描述符持有的 inode 解析，不按名字），并用 `O_DIRECTORY|O_NOFOLLOW` 在**遇到的那一刻**拒绝任何链接。某一段被打开之后再改名，对已经拿在手里的描述符没有任何影响；读取用的也是这条链末端的描述符本身。没有这套内核锚定（没有 procfs）时，**具名关闭**资产冻结（`path_anchoring_unavailable`），不退回到一个自己都不敢担保的按名校验。
**权限边界与残余（分开说）**：① 这条链只用进程**本来就有的**上传根读权限，**没有、也不需要扩大任何文件系统访问权限**，没有引入任何通用文件沙箱或新依赖。② 已复现的那条路径需要的是**既有归属行 + 在上传根里改写目录的能力**，**不是**数据库写权限——所以不能用"有库写权限就不管"把它带过。修后，改写目录**不再**能把读取重定向：链接在被走到时就拒，改名对描述符无效。③ 仍然绕得过去的只剩**在上传根里放进一个真实的目录项**（硬链接到根外文件，或把根外目录 bind mount 进来）**并且**让归属行指向它：前者需要以应用账号拿到 shell（那时他本来就能读到这些字节，不构成越权升级），后者需要管理员。HTTP 上的学生做不到其中任何一步——编辑器没有上传口，上传端点只会把文件写进 `chat-images/YYYY-MM/` 这类由服务端决定的目录名。④ 上传根**本身**的路径是部署配置（不是攻击者输入），按配置打开；它之下的每一段都按上面的方式走。
...
**逐次保存的历史，到哪为止**：耐久记录每一次观察到的保存的是**计数器**——`real_save_count`、`last_real_save_at` 与 `write_seq`（每次来源写入 +1）。内容事件（`artifact.updated`）是**每一次被对账的变化**一条：若 A→B→A 三次保存在任何对账之前发生，中间的字节已经不存在了，只会得到一条净变化事件（净内容与上次投影相同时，连这一条都没有，只有证据从 `none` 变 `observed` 那一条）。**所以不能把"最终状态补齐了"说成"每一次历史变化都已恢复"**：edu 想数保存次数看计数器，想看变化看事件，两者口径不同。计数器记的也只是**被观察到**的保存——保存写成功而钩子没写成（进程在两者之间崩溃）的那一次不会被计数，投影的内容仍由自愈通道追平，但它作为"一次保存"已经无从证明。
...
**对账顺序屏障（真实 MySQL 账本）**：`python3 dev/p09-lab/reconcile-order.py`——一次性 `mysql:8.0` + knex 执行的候选迁移（001 建表、002 为既有库补列）+ 真实 store/service/snapshot reader，来源项目是内存夹具，屏障是对来源读取加的闸门和插在读取边界里的一次保存。四条：迟到对账不得覆盖新保存、迟到对账不得清掉新标记、清扫与读取之间的保存不得被当作完整、以及合并保存的历史口径。**修复前先按当时的固定提交 `0c3b6d0` 复现（`order-20260922T150651Z`，三条全中），修复后同一脚本转为期望行为（`order-20260922T151632Z`，绑 `3a0c429`、`source_dirty:false`）**；两次运行的 `result.json` 都保留。002 迁移另有专门的重复执行检查 `python3 dev/p09-lab/migration-replay.py`（同样一次性 MySQL，六个库）：全新库重复 `up`、旧库缺双列、**已有列且欠账（7/3）重复 `up`**、只剩 `write_seq` 的半状态、只剩 `applied_write_seq` 的半状态、以及"迁移留下的欠账由真实 service 清扫消除"。**先按 `61df919` 复现（`migration-20260922T153011Z`：4/1→4/4、7/3→7/7、半状态被无证对齐），再修**。修后的规则：两列都在就**什么都不动**；都缺就**一条语句**同时补上（没有半状态窗口）、停在 0/0 不宣布任何事；只剩 `write_seq` 就补 `applied_write_seq=0`，让有写入的作品保持欠账；只剩 `applied_write_seq` 就**先给每个活跃作品打上待对账标记、再**丢弃这个无法验证的声明——**迁移只加缺的东西，绝不替业务对账宣布完成**，也绝不清除已有标记。

**中断出口**：修复本身也可能被打断，所以每个**已提交**的步骤都必须留下欠账信号。`applied_write_seq > write_seq` 在正常运行中不可能出现（服务只会存它刚采样的号），因此它就是"修复被中断"的指纹：`up` 一进来先找它，找到就把修复做完，找不到才什么都不做（7/3 这类普通欠账仍然一个字节都不写）。修复的两条语句是**先打标记、后丢声明**，所以中断在任一提交边界上都留下标记或这个指纹。`down` 同理：删列会把"只存在于计数里的欠账"一并删掉，所以它**先把欠账写成待对账标记**再删列。**唯一救不回来的**是更早那版候选已经造成的 `0/0 且无标记`——它与一个本来就同步的作品无法区分，只能由服务的自愈核对通道重新比对来源，本文照实说明，不假装能追认。
**执行门是可核条件，不是采样**：两次静默采样**不能**证明独占——采样结束后照样可以有写入或清扫。因此凡是**决定数据**的分支（半状态修复与 `down`）都在一个**服务器自己能确认的独占窗口**里执行：迁移固定一条连接、对账本表取 `LOCK TABLES … WRITE`，并在写任何东西之前用 `SHOW OPEN TABLES … In_use` 确认锁真的被持有；整个窗口内其他会话一律被挡住，与它用什么角色无关。取不到或确认不了就**具名拒绝**（`p09_exclusive_entry_unavailable` / `p09_exclusive_entry_unverified`）、一个字节都不改，并直接说明要给迁移账号补 `LOCK TABLES` 权限。那段 600ms 采样降级为**活动检测**，只用来描述拒绝，**不授权任何事**。纯追加 DDL（两列都缺/只缺一列）不做数据决定，不需要这个窗口。**实测而非假设**：`migration-replay.py` 持锁期间第二条连接的写入被挡住（跨 DDL 与 DML），锁释放后才落地；权限不足的账号跑同一条修复会被具名拒绝且表结构与行都没变。**迁移不自行停任何东西，也不把停用当授权；默认关闭也不证明每个部署都停了写。**

定稿运行：首次为 `run-20260922T144849Z`（SHA `3892d981…`，绑 `c3f377a`、`source_dirty:false`）；对账顺序修复后重跑 `run-20260922T151656Z`（SHA `9c3101f7d26c9fecb486d9c3ac3d9ff6be26a5279c90da224337ce1b6d7fb329`，绑 `3a0c429`，`source_dirty:true`——当时未提交的只有本文件这一处文档改动，没有实现改动），同样 21 个场景、21 条 checks、19 张截图。运行编号由紧随其后的文档提交写入，因此该提交本身不在运行的绑定范围内——这是记录顺序，不是未验证的改动。
