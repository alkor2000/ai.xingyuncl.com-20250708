...
前端面板 14 例全过（含"丢答复只显示未知、不显示拒绝、也不自己重发"）。

**真实浏览器实验** `dev/e09-submit-lab/check.py`（证据 `storage/private/e09-submit/run-20260924T093624Z/`，
19 张截图，15 条裁决全过）：
...
**反例那一格怎么造的**：替身先把这次提交记成已落库（流水上带 `committed:true`），再拖过本侧 4 秒绝对预算
才断开连接——答复丢在路上。裁决同时要求：对面确有一条已落库、面板只显示"暂时无法确认是否提交成功"、
不出现"已交"也不出现"没有交上"、提示里指向去 edu 核对，并且**再等 6 秒也没有第二条 `/submissions`、
替身也没有被第二次叫**。注意这只证明**本侧的显示与不重试**；真实 edu 的落库时机仍未验。
