...
- **新增一个学生路由**（上表），登录态校验与既有路由同源；请求体必须为空，多一个字段就按 `invalid_request` 拒；
  回答带三分的 `outcome`（`submitted` / `refused` / `unknown`）。
...
3. **未知既不是成功也不是拒绝，更不是"没交上"**：超时、答复丢失、看不懂的答复、没带 edu 自己码的 5xx，
   都只说**"暂时无法确认是否提交成功"**——对面可能已经落库。面板让学生先去 edu 的作业页核对提交记录，
   明确"不要用取消关联"（它不撤回任何已提交的内容），并说明再点一次是**另一次提交动作**。
   本侧不自动重试，也**不声称没有占用提交次数**。

三者在接口上就是分开的，不靠文案区分：`POST …/submissions` 的回答带 `outcome: 'submitted' | 'refused' | 'unknown'`，
面板按它渲染三种框（`p09-submitted` / `p09-submit-refusal` / `p09-submit-unknown`）。

**最后一跳同样按未知处理**：如果答复在"实践→浏览器"这一段断了（传输失败、没有本侧错误码的网关错误、
或者 2xx 正文读不出来），按下去的那一次可能已经在 edu 落库了，所以面板进**同一个未知框**，
而不是通用的"连接中断，请稍后重试"，也不静默清空。只有本侧在**外调之前**就给出的具名错误
（`link_unavailable` / `assignment_ref_missing` / `submit_unconfigured` / `invalid_request` / 未登录等）
仍按名报错——那几种确实一个字节都没发出去。这条改动只在交作业动作里，读取、保存、关联的通用错误处理未改。

**再点一次的边界**（按 edu 固定源码写清，不新造协议）：edu 的幂等键由**当次 change_no** 导出，所以
"再点一次"是**另一次提交动作**——没改过稿时对面可能认作同一次，改过稿就是另一个版本；
**算不算、算第几次都由 edu 判定**，本侧不承诺"恢复原来那一次"，也不承诺"不会多占一次"。
当前接口没有"回查上一次结果"的读接口，所以结果未知时本侧**保留未知**，只给出可实际走的核对方式
（去 edu 的作业页看提交记录），不造查询按钮、不造确认态。
...
- ~~拒绝答复里 `retryable` 的确切线形字段名未在固定导出中出现~~ **已闭合（2026-09-24 修订）**：
  edu 补出了原 23 文件漏掉的 `ailabpku-go/internal/handlers/e09_eligibility.go`
  （固定 07aa2b06…，文件 sha256 `0a7d3292…`，manifest sha256 `32558586…`）。
  `error.retryable` 是与 `code`/`message` 同级的真实布尔，edu 在 503 与任何 ≥500 时置 true。
  本侧改为**以该字段为准**（缺失时才回落到码表），并且记住它的边界：
  **retryable 只说明可以再试，不能证明对面没有落库**——丢答复与未识别的 5xx 仍按未知语义。
...
**受影响的服务/路由测试**（本候选树）：`websiteArtifact*` 与 `routes/websiteArtifacts` 共 7 个套件 105 例全过，
其中 `websiteArtifactSubmit.test.js` 13 例覆盖：事实取自账本行、他人 link 拒绝且**零外调**、
未关联/课次编号为空在外调前就拒、未配置通道按名拒绝、双击只发一次、拒绝原句透传、
逾期为终态而故障可重试、**edu 真实的 `error.retryable` 布尔说了算**、
**答复丢失是 unknown 而不是 refused**、**不带 edu 自己码的 5xx 也是 unknown**、
四种伪成功与一张 HTML 错误页都不被当成功、转达不写账本、生产下 http 端点与短密钥被拒、默认超时 8000ms。
前端面板 18 例全过，其中 4 例专盯最后一跳：传输被拒、没有本侧码的 504、2xx 正文读不出来都进未知框，
而外调之前的具名错误仍按名报错。**这 4 例是冲着旧代码写的**：把面板换回 d00c982 那一版跑，
前 3 例立刻失败（页面显示的是 `htmlEditor.p09.error.network_error`，找不到 `p09-submit-unknown`），
失败原文留在 `storage/private/e09-submit/pre-fix-panel-failures.txt`。

**真实浏览器实验** `dev/e09-submit-lab/check.py`（证据 `storage/private/e09-submit/run-20260924T095554Z/`，
21 张截图，19 条裁决全过）：
...
| 断连 | "暂时无法确认是否提交成功"＋去 edu 核对 | 1 / 1 |
| 超时（对面挂住不答，走本侧 4s 绝对截止） | 同上未知提示 | 1 / 1 |
| **对面已落库、答复丢了**（反例） | 同上未知提示；成功框与"没有交上"都不出现 | 1 / 1，等 6 秒无第二条 |
| 伪成功（200 但缺固定版字段） | "暂时无法确认是否提交成功" | 1 / 1 |
| **答复丢在实践→浏览器**（反例，整条丢） | 未知提示；不出现"连接中断" | 1 / 1，等 6 秒无第二条 |
| **答复截断在实践→浏览器**（反例，307 字节截到 102） | 同上 | 1 / 1，等 6 秒无第二条 |
...
**两个反例分别断在哪一跳**（这是它们的区别，不是重复）：

1. **edu→实践**：替身先把这次提交记成已落库（流水带 `committed:true`），再拖过本侧 4 秒绝对预算才断开。
2. **实践→浏览器**：用 Playwright 路由拦截，让请求**真的打到后端**（转达真的发生、替身真的落库），
   只把回给页面的答复丢掉或截断。证据里记了断点：后端原答复 `200 / 307 字节 / "submitted":true,"revision_no":9`，
   整条丢时 `cut_at=0`，截断时 `cut_at=102`。纯离线（请求根本没发出去）不算这个反例。

两者的裁决一样严：对面确有一条已落库、面板只显示"暂时无法确认是否提交成功"、不出现"已交"也不出现"没有交上"、
不出现"连接中断"、提示里指向去 edu 核对，并且**再等 6 秒也没有第二条 `/submissions`、替身也没有被第二次叫**。
注意这只证明**本侧的显示与不重试**；真实 edu 的落库时机仍未验。
