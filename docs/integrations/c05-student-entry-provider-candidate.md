...
consume 在**一个事务、一条连接**上重读现态并逐条核对，然后在同一个事务里签发令牌、写入会话上下文行——
要么会话连同上下文一起存在，要么什么都没有。身份所需的整份用户对象也走**该事务自己的 query**
（`User.findById(id, query)` 的可选第二参），不再向全局连接池要第二条连接：那条连接正是本事务握着的，
应用池上限为 1 时会死等自己（已在真实 MySQL 上复现，见 §8）。组行的核对是 `FOR UPDATE`，
按既有锁序（先用户行、再按 id 升序锁组行），所以"核过之后、提交之前把学校停用"只能排队等这次兑换结束。
配置来源分层如实说明：`school_source: 'config'` 的映射来自**请求级配置快照**（每请求读一次
`system_settings`），`'database'` 的映射在事务内读，而决定放不放行的那次组行核对始终在行锁下完成。中途任何一条不成立都是具名拒绝且不发会话：
...
`down` 具名拒绝、过期后干净删表。27 条判定全通过，修复前的失败报告保留在同目录的上一轮 `gates-*`。

第三轮 `dev/c05-lab/tx-pool.py`（真实 MySQL/Redis/HTTP，**应用连接池上限设为 1，只在隔离环境里设，
不改候选的生产池上限**）：先复现——consume 15 秒不返回、服务端只看得到应用账号的**一条** `Sleep` 连接
且 `innodb_trx` 里有一个 RUNNING 事务锁着 1 行、会话数 0、整个后端要重启才恢复（等待发生在应用自己的
连接池队列里，所以数据库层面看不到任何锁等待）；修复后同样池 1：0.03 秒返回 200、一条会话；
四张不同票在池 1 上全部成功；同一张票三个并发只有一次 200 且只有一行会话；
组停用在一次兑换进行中被挡了 8.02 秒（等这次兑换结束才生效）。
回退边界：活会话/全过期/全撤销都被 `c05_sessions_not_empty` 挡住、空表才回退；
另一条连接占着表时迁移具名拒绝且表仍在；持 `LOCK TABLES … WRITE` 期间别的连接写不进去、解锁后才落地；
并发写入下 `down` 拒绝且表完好（58 次写入全部先于计数落地）。
一处如实记录**未观察到**：为了看"锁已持有之后才发起的写入"，从第三条连接轮询 `In_use` 没能抓到那个窗口
（LOCK→计数→DROP 太短），因此该臂记为未观察，不当作已证明；写入被挡的直接证据与拒绝臂都在报告里。
12 条判定全通过。
