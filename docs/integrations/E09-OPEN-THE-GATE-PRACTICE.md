...
- 因此这四个文件必须由**运维显式执行**，两种做法二选一：
  - 把四个文件**临时**复制进一个只含它们的目录，用 knex 指向该目录执行（不要复制进 `backend/migrations/`）；
  - 或按文件内的 `exports.up` 自行以同等 SQL 执行。
- **不要把候选文件移进 `backend/migrations/`**——那等于让下一次发布自动建表，属于"晋级"，需要单独授权。
...
2. （实践）学生从作业页点进来 → 应当直接落到实践平台且已登录；地址栏里的一次性票据**立刻消失**。
3. （实践）打开网页编辑器 → 项目下方出现「教学任务作品」面板 → 选自己的项目与入口页 → 关联。
4. （实践）改一行内容并**保存**一次 → 面板状态变成"制作中"，能看到保存时间。
5. （实践）点"生成评阅版本" → 得到一版固定版。（老师看到的就是这一版。）
6. （edu）老师打开那一版 → 能看到内容与图片；学生再改稿，老师那边**仍是旧版**。**待准备：** 同第 1 步。
