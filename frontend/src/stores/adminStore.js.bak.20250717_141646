...
const useAdminStore = create((set) => ({
  ...
  setUserCreditsExpire: async (userId, data) => {
    try {
      const response = await apiClient.put(`/admin/users/${userId}/credits/expire`, data)
      return response.data.data
    } catch (error) {
      console.error('设置用户积分有效期失败:', error)
      throw error
    }
  },
