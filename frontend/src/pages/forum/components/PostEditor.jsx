...
import {
  Typography, Input, Select, Button, Space, Upload, message, Card
} from 'antd';
import {
  ArrowLeftOutlined, PictureOutlined, PaperClipOutlined,
...
const PostEditor = ({
  ...
  const { createPost, updatePost, uploadImages, uploadFiles } = useForumStore();
  ...
  const handleImageUpload = async (info) => {
    ...
    try {
      ...
      const newImages = result.map(att => ({
        id: att.id,
        name: att.file_name,
        path: att.file_path,
    ...
    <div className="post-editor">
      <div className="editor-header">
        ...
        <Title level={4} style={{ margin: 0 }}>
          {isEditing ? t('forum.post.edit') : t('forum.post.new')}
        </Title>
      </div>
      ...
      <Card className="editor-card">
        ...
        {!isEditing && (
          <div className="editor-field">
            ...
            <Select
              value={boardId}
              onChange={setBoardId}
              placeholder={t('forum.post.form.boardRequired')}
              style={{ width: '100%', marginTop: 8 }}
              options={boards.filter(b => b.is_active).map(b => ({
                label: b.name,
                value: b.id
              }))}
            />
        ...
        <div className="editor-field">
          ...
          <Input
            ...
            placeholder={t('forum.post.form.titlePlaceholder')}
            maxLength={200}
            showCount
            style={{ marginTop: 8 }}
          />
        ...
        <div className="editor-field">
          ...
          <TextArea
            ...
            placeholder={t('forum.post.form.contentPlaceholder')}
              ...
              <div className="uploaded-preview" style={{ marginTop: 12 }}>
                {uploadedImages.map(img => (
                  <div key={img.id} className="preview-item">
                    ...
                    <span className="preview-remove" onClick={() => removeImage(img.id)}>
                      <DeleteOutlined />
                    </span>
                  </div>
                ))}
