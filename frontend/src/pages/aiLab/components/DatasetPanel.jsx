...
const DatasetPanel = ({ dataset, samples, onAddClass, onRenameClass, onDeleteSample, canEdit }) => {
    ...
    <div className="ailab-dataset">
      {canEdit && (
        <Space.Compact style={{ width: '100%', maxWidth: 420, marginBottom: 12 }}>
          <Input placeholder={t('aiLab.dataset.newClassPlaceholder')} value={newLabel} maxLength={30} onChange={(e) => setNewLabel(e.target.value)} onPressEnter={handleAdd} />
        ...
        return (
          <div className="ailab-class-block" key={cls.key}>
            <div className="ailab-class-head">
              ...
              <Tooltip title={t('aiLab.dataset.holdoutHidden')}>
                <Tag icon={<LockOutlined />}>{t('aiLab.split.holdout')} {counts.holdout?.[cls.key] || 0}</Tag>
              </Tooltip>
            ...
            <div className="ailab-sample-grid">
              ...
              {!list.length && <div className="ailab-sample-empty">{t('aiLab.dataset.noTrainSamples')}</div>}
            </div>
          </div>
        )
      })}
      <Modal
        title={t('aiLab.dataset.renameClass')}
        open={!!editing}
        onCancel={() => setEditing(null)}
        onOk={async () => { await onRenameClass(editing.key, editing.label.trim()); setEditing(null) }}
