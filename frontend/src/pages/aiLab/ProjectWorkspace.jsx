 ...
 * AI训练专区 · 实验工作台
 *
 * 一个项目 = 一个实验：预测 → 采集 → 锁定留出集 → 训练 → 留出测试 → 换条件测试 →
 * 错误分析 → 再训练对照 → 模型卡。左侧步骤栏按真实数据判断完成状态；
 * 每一步的关键动作都写一条过程事实（见 aiLabStore.recordEvent）。
...
const ProjectWorkspace = () => {
  ...
  const {
    project, datasets, models, samplesByDataset, projectLoading, tasks,
    openProject, fetchTasks, fetchSamples, uploadSamples, deleteSample, lockSplit,
  ...
  const steps = task?.steps || DEFAULT_STEPS
  ...
  const sections = {
    ...
    collect: () => section('collect', t('aiLab.step.collect'), t('aiLab.section.collectHint', { min: minPerClass }), (
      <>
        ...
        {canEdit && classes.length > 0 && <CapturePanel dataset={dataset} classes={classes} split="train" onUpload={handleTrainUpload} />}
      </>
    )),
    lock: () => section('lock', t('aiLab.step.lock'), t('aiLab.section.lockHint'), (
      <Space direction="vertical" style={{ width: '100%' }}>
        <Text>{t('aiLab.lock.counts', { train: trainTotal, holdout: Object.values(counts.holdout || {}).reduce((a, b) => a + b, 0) })}</Text>
    ...
    train: () => section('train', t('aiLab.step.train'), t('aiLab.section.trainHint'), <TrainPanel dataset={dataset} models={models} minPerClass={minPerClass} canEdit={canEdit} />),
    test_holdout: () => section('test_holdout', t('aiLab.step.test_holdout'), t('aiLab.section.testHint'), <EvaluatePanel dataset={dataset} models={models} labelOf={labelOf} canEdit={canEdit} />),
    test_shift: () => section('test_shift', t('aiLab.step.test_shift'), t('aiLab.section.shiftHint'), <Text type="secondary">{t('aiLab.section.shiftWhere')}</Text>),
    ...
    errors: () => section('errors', t('aiLab.step.errors'), t('aiLab.section.errorsHint'), (
      <Space>
    ...
    iterate: () => section('iterate', t('aiLab.step.iterate'), t('aiLab.section.iterateHint'), <ComparePanel models={models} />),
