 ...
 * 教学管理后台组件（全局授权管理增强版）
 * 新增功能：
 * 1. Tab结构：智能教学授权管理 + 模块管理 + 分组管理 + 页面头部配置
 * 2. 智能教学授权管理：全局三级授权（组→标签→用户）
 * 3. 分组管理：创建、编辑、删除、排序、批量操作
 * 权限：仅超级管理员可见
 */
...
// 导入全局授权管理组件
import ModuleAuthorizationManagement from './ModuleAuthorizationManagement';
...
const TeachingManagement = () => {
  ...
  const tabItems = [
    {
      ...
      children: <ModuleAuthorizationManagement />
    },
    {
      key: 'modules',
      label: (
  ...
  return (
    <div>
      ...
      {/* Tab区域（默认显示智能教学授权管理Tab）*/}
      <Card>
        <Tabs items={tabItems} defaultActiveKey="authorization" />
      </Card>
